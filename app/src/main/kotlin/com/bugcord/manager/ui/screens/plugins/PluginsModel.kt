package com.bugcord.manager.ui.screens.plugins
...
import com.bugcord.manager.ui.screens.plugins.model.PluginItem
import com.bugcord.manager.ui.screens.plugins.model.PluginManifest
...
class PluginsModel(
    ...
    fun setSafeMode(safeMode: Boolean) = screenModelScope.launchIO {
        try {
            editBugcordSettings {
                put(JsonPrimitive("AC_bugcord_safe_mode_enabled"), JsonPrimitive(safeMode))
            }
    ...
    private fun loadSafeMode() = screenModelScope.launchIO {
        ...
        data class SafeModeSettings(
            @SerialName("AC_bugcord_safe_mode_enabled")
            val safeMode: Boolean = false,
        )

        pluginsSafeMode.value = readBugcordSettings<SafeModeSettings>()?.safeMode ?: false
    }

    private suspend fun loadPluginsEnabled() {
        val pluginToggles = readBugcordSettings<Map<JsonPrimitive, JsonElement>>()
    ...
    /**
     * Reads Bugcord core's settings, applies [block] to it, and writes it back.
     */
    private suspend fun editBugcordSettings(block: (MutableMap<JsonPrimitive, JsonElement>).() -> Unit) {
        SETTINGS_MUTEX.withLock {
            val settings = try {
                ...
                Log.e(BuildConfig.TAG, "Bugcord settings are corrupted!", e)
                mutableMapOf()
            }
    ...
    /**
     * Reads Bugcord core's settings and parses it into a specific model.
     * This should not be used for future writes.
     *
     * @return The parsed settings model, or null if settings are missing or corrupt.
     */
    private suspend inline fun <reified T> readBugcordSettings(): T? {
    ...
    private companion object {
        /**
         * Global lock on the main Bugcord settings.
         */
