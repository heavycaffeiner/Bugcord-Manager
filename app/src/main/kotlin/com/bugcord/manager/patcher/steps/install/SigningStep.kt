...
// TODO: prompt user to uninstall Bugcord if signing keystore is unavailable/corrupt and CorePatch isn't installed
...
class SigningStep(
    ...
    override suspend fun execute(container: StepRunner) {
        ...
        ZipWriter(apk, /* append = */ true).use { zip ->
            zip.writeEntry("bugcord.keystore", keystoreBytes)
        }
     ...
     *
     * 1. If the specified Bugcord installation for [packageName] exists,
    ...
    private fun getKeystore(packageName: String): Pair<KeyStore, ByteArray> {
        val embeddedKeystoreRaw = try {
            ...
            ZipReader(applicationInfo.publicSourceDir)
                .use { it.openEntry("bugcord.keystore")?.read() }
        } catch (_: Exception) {
            null
        }
        ...
        if (embeddedKeystoreRaw != null) {
            try {
                ...
                throw Exception("Embedded existing signing key is corrupted! Please uninstall Bugcord and retry!", e)
            }
        }
    ...
    private fun createKeystore(password: CharArray): ByteArray {
        ...
        val certificate = X509v3CertificateBuilder(
            /* issuer = */ X500Name("CN=Bugcord Manager"),
            ...
            /* subject = */ X500Name("CN=Bugcord Manager"),
    ...
    private companion object {
         ...
         * `/storage/emulated/0/Bugcord/ks.keystore`
         */
