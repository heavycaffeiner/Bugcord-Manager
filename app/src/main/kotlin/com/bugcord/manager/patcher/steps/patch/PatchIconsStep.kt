...
import com.bugcord.manager.patcher.util.*
import com.bugcord.manager.patcher.util.ArscUtil.addColorResource
import com.bugcord.manager.patcher.util.ArscUtil.addResource
import com.bugcord.manager.patcher.util.ArscUtil.getMainArscChunk
import com.bugcord.manager.patcher.util.ArscUtil.getPackageChunk
import com.bugcord.manager.patcher.util.ArscUtil.getResourceFileName
import com.bugcord.manager.patcher.util.ArscUtil.getResourceFileNames
import com.bugcord.manager.ui.screens.patchopts.PatchOptions
import com.bugcord.manager.ui.screens.patchopts.PatchOptions.IconReplacement
import com.bugcord.manager.util.getRawBytes
...
class PatchIconsStep(private val options: PatchOptions) : Step(), KoinComponent {
    ...
    private fun patchAdaptiveIcons(
        ...
        val monochromePath = when (options.iconReplacement) {
            is IconReplacement.CustomImage -> "res/ic_bugcord_monochrome.png"
            else -> "res/ic_bugcord_monochrome.xml"
        }
        ...
        if (isMonochromeIconsAvailable) {
            ...
            monochromeIcon = arsc.getPackageChunk().addResource(
                typeName = "drawable",
                resourceName = "ic_bugcord_monochrome",
    ...
    private fun patchRawIcons(
        ...
        val backgroundColor = when (options.iconReplacement) {
            IconReplacement.Original -> IconReplacement.BugcordColor
